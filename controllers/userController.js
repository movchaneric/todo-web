...
exports.postUserRegister = async (req, res, next) => {
    ...
    try {
        ...
        if (userFound) {
            ...
            const hashedPassword = await bcrypt.hash(password, PASSWORD_SECURITY_LEVEL);
...
exports.postResetForm = async (req, res, next) => {
    ...
    try{
        const hashedPassword = await bcrypt.hash(newPassword, PASSWORD_SECURITY_LEVEL)
...
exports.getProfilePage = async (req, res, next) => {
    ...
    try{
        ...
        try {
            const numOfTasksDone = await Task.count({ userId: userIdConnceted, status: 'Done' })
            try{
